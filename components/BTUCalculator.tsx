...
const BTUCalculator: React.FC = () => {
  ...
  const rawTotal = (area * baseBTU) + peopleBTU;

  const capacities = [9000, 12000, 18000, 24000, 30000, 36000, 48000, 60000];
  const recommended = area > 0
    ? (capacities.find(cap => cap >= rawTotal) || capacities[capacities.length - 1])
    : null;
            ...
            {recommended && (
              <div className="bg-gradient-to-br from-primary to-secondary p-8 rounded-2xl shadow-lg text-white animate-fade-in transform hover:scale-105 transition-transform">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-5xl font-extrabold">{recommended}</span>
